...
import { addPnlRecord, clearPnlHistory, loadPnlHistory } from "../lib/pnlHistory";
...
const priceTickFor = (symbol, fallbackPrice) => {
...
const roundPriceToTick = (symbol, price, fallbackPrice) => {
...
const resolveTrailTp = (dir, price, tp, oneR) => {
    const baseTp = normalizeTp(tp);
    if (!baseTp)
        return { tp: undefined, bumped: false };
    const priceOk = dir > 0 ? price < baseTp : price > baseTp;
    if (priceOk)
        return { tp: baseTp, bumped: false };
...
const SOFT_SPREAD_BPS = {
    BTCUSDT: 4,
    ETHUSDT: 6,
    SOLUSDT: 8,
    ADAUSDT: 10,
};
const BREAK_BUFFER_BPS = {
    BTCUSDT: 2,
    ETHUSDT: 3,
    SOLUSDT: 5,
    ADAUSDT: 8,
};
const ATR_SWEET_SPOT = {
...
const maxQtyForSymbol = (symbol) => QTY_LIMITS[symbol]?.max ?? Number.POSITIVE_INFINITY;
const qtyStepForSymbol = (symbol) => QTY_STEPS[symbol] ?? 0.001;
...
export const useTradingBot = (mode, useTestnet, authToken) => {
    ...
    const protectionTargetsRef = useRef({});
            ...
            try {
                ...
                try {
                    ...
                    if (pnlRes.ok) {
                        ...
                        const filteredRecords = filtered.map((rec) => ({
                            symbol: rec.symbol,
                            pnl: rec.pnl,
    ...
    const cancelOrderByLinkId = useCallback(async (symbol, orderLinkId) => {
        if (!authToken)
            return;
    ...
    const placeReduceOnlyLimit = useCallback(async (symbol, side, qty, price, timeInForce, orderLinkId, reason) => {
        if (!authToken)
            return false;
        const step = qtyStepForSymbol(symbol);
        const safeQty = roundDownToStep(qty, step);
        if (!Number.isFinite(safeQty) || safeQty < step)
            return false;
    ...
    const placeReduceOnlyExit = useCallback(async (symbol, side, qty, reason) => {
        ...
        if (!Number.isFinite(remainingQty) || remainingQty < step)
            return true;
        try {
            ...
            for (const p of positions) {
                ...
                if (expectedProtection) {
                    ...
                    if (now - lastVerifyAt >= PROTECTION_VERIFY_COOLDOWN_MS) {
                        ...
                        if (slMismatch || tpMismatch || tsMismatch) {
                            protectionVerifyRef.current[p.symbol] = now;
                            const ok = await commitProtection(`verify-${p.id}`, p.symbol, expectedProtection.sl ?? p.sl, expectedProtection.tp ?? p.tp, expectedProtection.trailingStop ?? p.trailingStop);
                            if (ok) {
                ...
                if (trendOk && Number.isFinite(p.tp) && profitR >= TP_EXTEND_MIN_R) {
                    ...
                    if (profitR >= lastShiftR + TP_EXTEND_STEP_R) {
                        const newTp = p.tp + dir * TP_EXTEND_STEP_R * oneR;
                        const safeTp = dir > 0 ? newTp > p.tp && newTp > price + oneR * 0.1 : newTp < p.tp && newTp < price - oneR * 0.1;
                        if (safeTp) {
    ...
    useEffect(() => {
        ...
        const softSpreadBpsFor = (symbol) => SOFT_SPREAD_BPS[symbol] ?? Math.max(1, Math.round(hardSpreadBpsFor(symbol) * 0.7));
        const breakBufferBpsFor = (symbol) => BREAK_BUFFER_BPS[symbol] ?? 3;
        const atrSweetSpotFor = (symbol) => ATR_SWEET_SPOT[symbol] ?? { low: 0.001, high: 0.004 };
        ...
        const qualityScoreFor = (symbol, ctx) => {
            ...
            const htfDistAtr = ctx.htfAtr > 0 ? Math.abs(ctx.htfClose - ctx.htfEma200) / ctx.htfAtr : undefined;
            if (htfScore > 0 && Number.isFinite(htfDistAtr) && htfDistAtr > 2.5) {
                htfScore = Math.max(0, htfScore - 5);
            }
            ...
            if (Number.isFinite(distToEma)) {
                ...
                else if (distToEma <= LATE_ENTRY_ATR) {
                    pullbackScore = 10;
                }
            }
            let microScore = 0;
            ...
            if (top.missing > 0) {
                switch (top.name) {
                    case "Pullback":
                        topReason = Number.isFinite(distToEma) && distToEma > LATE_ENTRY_ATR
                            ? "Late entry"
                            : "Pullback weak";
                        break;
                    case "Break":
        ...
        const isSymbolAtMaxQty = (symbol, sizes) => {
            const maxQty = maxQtyForSymbol(symbol);
            if (!Number.isFinite(maxQty))
                return false;
            return (sizes[symbol] || 0) >= maxQty - 1e-8;
        };
        const hasMaxedPositions = (stats) => stats.count >= 2 && Array.from(stats.symbols).every((sym) => isSymbolAtMaxQty(sym, stats.sizes));
        ...
        const buildRepriceId = (base, attempt) => {
            const suffix = `-R${attempt}`;
            const next = `${base}${suffix}`;
            return next.length <= 36 ? next : next.slice(0, 36);
        };
        const computeEntryLimit = (st, side) => {
            if (!st.instrument || !st.bbo)
                return null;
            ...
            if (side === "Buy" && limit >= st.bbo.ask)
                return null;
            if (side === "Sell" && limit <= st.bbo.bid)
                return null;
            return limit;
        };
    ...
    const performTrade = async (signalId) => {
        ...
        const activeCount = new Set(activePositionsRef.current
            .filter((p) => Math.abs(Number(p.size ?? p.qty ?? 0)) > 0)
            .map((p) => p.symbol)).size;
        ...
        let finalSl = Number.isFinite(finalSlRaw) ? roundToTick(finalSlRaw, tick) : undefined;
        if (Number.isFinite(finalSl)) {
            if (isBuy && finalSl >= safeEntry - tick)
                finalSl = roundToTick(safeEntry - tick, tick);
            if (!isBuy && finalSl <= safeEntry + tick)
                finalSl = roundToTick(safeEntry + tick, tick);
        }
        if (!Number.isFinite(finalSl)) {
        ...
        if (isBuy && tp2 <= safeEntry + tick)
            tp2 = roundToTick(safeEntry + tick, tick);
        if (!isBuy && tp2 >= safeEntry - tick)
            tp2 = roundToTick(safeEntry - tick, tick);
        let finalTp = Number.isFinite(finalTpRaw) ? roundToTick(finalTpRaw, tick) : undefined;
        if (Number.isFinite(finalTp)) {
            const invalid = isBuy ? finalTp <= safeEntry + tick : finalTp >= safeEntry - tick;
            if (invalid)
                finalTp = undefined;
        }
        const stopLossValue = Number(finalSl ?? safeEntry ?? 0);
        const takeProfitValue = tp2;
        const protectionTp = Number.isFinite(finalTp) ? finalTp : tp2;
