...
export const useTradingBot = (mode, useTestnet, authToken) => {
    ...
    const performTrade = async (signalId) => {
        ...
        const recoveryMult = portfolioState.currentDrawdown >= 0.15 &&
            portfolioState.currentDrawdown < settings.maxDrawdownPercent
            ? 0.5
            : 1;
        const riskPctWithMult = Math.min(Math.max(0.005, baseRiskPct * volMult * recoveryMult * (settings.positionSizingMultiplier || 1)), 0.04);
        if (settings.haltOnDailyLoss &&
            portfolioState.dailyPnl <= -portfolioState.maxDailyLoss) {
            const now = Date.now();
            if (!dailyHaltAtRef.current)
                dailyHaltAtRef.current = now;
            ...
            if (now < haltUntil) {
                addLog({
                    ...
                    message: `Trading halted: daily loss limit hit (${((portfolioState.maxDailyLoss * 100) /
                        Math.max(1, portfolioState.totalCapital)).toFixed(2)}%). Resume after cooldown.`,
                });
                return false;
            }
            else {
