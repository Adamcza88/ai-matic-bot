...
type StructureBias = "bullish" | "bearish" | null;
...
function findLastSwingBreak(candles: Candle[], bias: StructureBias, n = 2) {
    for (let i = candles.length - (n + 2); i >= n; i--) {
        if (bias === "bullish" && isSwingHigh(candles, i, n)) {
            ...
            if (afterClose) return { level, idx: i };
        }
...
function netRrr(entry: number, sl: number, tp: number, feePct: number) {
...
function buildZone(candles: Candle[], bosIdx: number, direction: "buy" | "sell") {
...
function evaluateMarketStructureWithReason(
    ...
    const bias4h = (() => {
        ...
        if (bosUp && (!bosDn || bosUp.idx > bosDn.idx)) return "bullish" as StructureBias;
    ...
    const choch15 = (() => {
        ...
        if (bias4h === "bullish" && bosUp) return "bullish";
...
function computePositionSizing(symbol: string, entry: number, sl: number, useTestnet: boolean): SizingResult {
...
export const useTradingBot = (
    ...
    const shouldAllowMarketEntry = (spreadPct: number, depthOk: boolean, momentumOk: boolean) => {
        if (spreadPct <= 0.0005 && depthOk && momentumOk) return true; // 0.05 %
        return false;
    };
...
function buildBotEngineCandidate(symbol: string, candles: Candle[]): RankedSignal | null {
    ...
    useEffect(() => {
        ...
        const isCancelSafeError = (err: any) => {
        ...
        const processSymbol = async (symbol: string, plannedAt: number): Promise<boolean> => {
            ...
            if (!flipped) gateFailures.push("ST flip");
            if (!emaOk) gateFailures.push("EMA pullback");
            if (!closeVsSt) gateFailures.push("Close vs ST");
            if (!htfProj) gateFailures.push("HTF line");
            if (!rvolOk) gateFailures.push("RVOL");
            if (!antiBreakout) gateFailures.push("Anti-breakout");
    ...
    const performTrade = async (signalId: string): Promise<boolean> => {
        ...
        // Risk engine sizing
        const sizing = computePositionSizing(symbol, safeEntry, finalSl as number, useTestnet);
        if (sizing.ok === false) {
