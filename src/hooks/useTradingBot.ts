...
export const useTradingBot = (
  ...
  const [aiModelState, setAiModelState] = useState({
    ...
    lastRetrain: new Date(Date.now() - 7 * 24 * 3600 * 1000).toISOString().split("T")[0],
    nextRetrain: new Date(Date.now() + 7 * 24 * 3600 * 1000).toISOString().split("T")[0],
  ...
  async function executeTrade(signalId: string) {
    ...
    const maxAlloc = portfolioState.totalCapital * settings.baseRiskPerTrade;
    ...
    if (portfolioState.allocatedCapital + notional > portfolioState.maxAllocatedCapital) {
      addLog({
        action: "RISK_BLOCK",
    ...
    let trailingDistance = oneR * 0.5; // 0.5R
    ...
    addLog({
      ...
      message: `Opened ${side.toUpperCase()} ${
        ...
        4
      )}, notional ≈ ${notional.toFixed(2)} USD, TS≈${trailingStopPct.toFixed(
        2
      )}% from 1R)`,
    });
  ...
  useEffect(() => {
    ...
    if (settingsRef.current.requireConfirmationInAuto && sig.risk < 0.65) return;
