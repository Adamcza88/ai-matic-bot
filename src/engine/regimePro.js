...
function baumWelch(obs, model, iterations = 5) {
  const T = obs.length;
  ...
  for (let iter = 0; iter < iterations; iter++) {
    ...
    const xi = logXi.map((row) => row.map((col) => col.map((v) => Math.exp(v))));
    pi = gamma[0].slice();
    for (let i = 0; i < N; i++) {
      const denom =
...
function normalizeStateByVol(means) {
...
export function computeHurst(closes, window = 100) {
...
export function computeChop(highs, lows, closes, period = 14) {
