...
export function updateOrderbook(symbol, bids, asks, isSnapshot) {
...
export function updateLiquidations(symbol, events) {
  const state = getState(symbol);
  const now = Date.now();
...
export function updateOpenInterest(symbol, oiValue) {
  const state = getState(symbol);
...
export function getOrderFlowSnapshot(symbol) {
  ...
  const icebergDetected =
    Number.isFinite(state.icebergUntil) && state.icebergUntil > Date.now();
