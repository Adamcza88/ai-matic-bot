import { State, Trend, resampleCandles, type Candle, type EngineDecision, type EngineSignal } from "./botEngine";
import { computeEma } from "./ta";

export enum AMDPhase {
...
type HtfBias = "bullish" | "bearish" | "none";
...
export type AmdContext = {
  phase: AMDPhase;
  bias: HtfBias;
  sessionNyDate: string;
  killzoneActive: boolean;
  killzoneName: "ASIA" | "LONDON" | "NY_AM" | "NONE";
  midnightOpen: number | null;
  accumulationRange: { high: number; low: number } | null;
  manipulation: {
    detected: boolean;
    low: number | null;
    high: number | null;
  ...
  fvg: (AmdFvg & { confirmed: boolean }) | null;
  targets: { tp1: number; tp2: number } | null;
  gates: {
    phaseSequence: boolean;
    killzoneActive: boolean;
    midnightOpenSet: boolean;
    asiaRangeValid: boolean;
    liquiditySweep: boolean;
    inversionFvgConfirm: boolean;
    targetModelValid: boolean;
  };
  details: string[];
};
...
const toNum = (value: string) => Number.parseInt(value, 10);

const toDateKey = (year: number, month: number, day: number) =>
  `${year}-${String(month).padStart(2, "0")}-${String(day).padStart(2, "0")}`;

const prevDateKey = (dateKey: string) => {
...
const resolveKillzoneName = (hourNy: number): AmdContext["killzoneName"] => {
  if (hourNy >= 20 && hourNy < 24) return "ASIA";
  if (hourNy >= 2 && hourNy < 5) return "LONDON";
  if (hourNy >= 8 && hourNy < 11) return "NY_AM";
  return "NONE";
};

const isManipulationKillzone = (hourNy: number) =>
  (hourNy >= 2 && hourNy < 5) || (hourNy >= 8 && hourNy < 11);

const resolveHtfBias = (candlesH1: Candle[]): HtfBias => {
  if (candlesH1.length < 220) return "none";
  ...
  if (lastClose > e50 && e50 > e200) return "bullish";
  if (lastClose < e50 && e50 < e200) return "bearish";
  return "none";
};

const findMidnightOpen = (candlesM15: Candle[], sessionDate: string): number | null => {
  ...
  if (exact && Number.isFinite(exact.open)) return exact.open;
...
const resolveAccumulationRange = (
  candlesM15: Candle[],
  asiaDate: string
...
const calculateTarget = (
  ...
  if (bias === "bullish") return manipLow + n * (manipHigh - manipLow);
  if (bias === "bearish") return manipHigh - n * (manipHigh - manipLow);
  return Number.NaN;
};

const detectFvgs = (candles: Candle[]): AmdFvg[] => {
  ...
  for (const fvg of out) {
    const start = candles.findIndex(
      (c) => (c.openTime ?? c.timestamp ?? 0) >= fvg.createdAt
    );
    if (start < 0) continue;
...
export function evaluateAiMaticAmdStrategyForSymbol(
  symbol: string,
  candles: Candle[]
): EngineDecision {
  ...
  let manipulationLow: number | null = null;
  let manipulationHigh: number | null = null;
  let manipulationTs: number | null = null;
  ...
  if (bias !== "none" && Number.isFinite(midnightOpen ?? Number.NaN) && accumulationRange) {
    ...
    for (let i = sessionManipCandles.length - 1; i >= 0; i--) {
      ...
      if (bias === "bullish") {
        if (
          candle.low < (midnightOpen as number) &&
        ...
        if (
          candle.high > (midnightOpen as number) &&
  ...
  const fvgCandidate = fvgs
    ...
    .reverse()
    .find((fvg) => {
      if (!Number.isFinite(manipulationTs ?? Number.NaN)) return false;
      if (fvg.createdAt < (manipulationTs as number)) return false;
      if (fvg.isMitigated) return false;
      if (bias === "bullish") return fvg.direction === "bearish";
      if (bias === "bearish") return fvg.direction === "bullish";
      return false;
    });
  const currentPrice = lastM1.close;
  const inversionConfirm = Boolean(
    fvgCandidate &&
      ((bias === "bullish" && currentPrice > fvgCandidate.top) ||
        (bias === "bearish" && currentPrice < fvgCandidate.bottom))
  );
  const fvgContext = fvgCandidate
    ? ({ ...fvgCandidate, confirmed: inversionConfirm } as const)
    : null;

  const tp1 =
    manipulationDetected && bias !== "none"
      ? calculateTarget(
          1,
          manipulationLow as number,
          manipulationHigh as number,
          bias
        )
      : Number.NaN;
  const tp2 =
  ...
  if (asiaRangeValid) phase = AMDPhase.ACCUMULATION;
  if (manipulationDetected) phase = AMDPhase.MANIPULATION;
  if (manipulationDetected && inversionConfirm && killzoneActive) {
    phase = AMDPhase.DISTRIBUTION;
  }

  const gates: AmdContext["gates"] = {
    phaseSequence:
      phase === AMDPhase.DISTRIBUTION && asiaRangeValid && manipulationDetected,
    killzoneActive,
  ...
  if (
    ...
    const sl = bias === "bullish" ? (manipulationLow as number) : (manipulationHigh as number);
    const stableKey = `${symbol}-amd-${sessionNyDate}-${bias}-${Math.round(
      manipulationTs as number
    )}`;
    signal = {
      id: stableKey,
      symbol,
      ...
      kind: "MEAN_REVERSION",
      risk: 0.6,
      message: `AMD ${phase} ${bias} | KZ ${killzoneName} | TP1 ${tp1.toFixed(
        4
      )} TP2 ${tp2.toFixed(4)}`,
