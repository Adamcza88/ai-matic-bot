...
function mergeTimeframeCandles(primary, supplemental) {
...
function detectWedgePop(args) {
    ...
    const isVolumeOk = Number.isFinite(volSma) &&
        volSma > 0 &&
...
function resolveSwingTrend(bars) {
    if (bars.length < 10)
        return "NONE";
    const highs = [];
    const lows = [];
    for (let i = 2; i < bars.length - 2; i += 1) {
        ...
        const isPivotHigh = high >= bars[i - 1].high &&
            high >= bars[i - 2].high &&
            high >= bars[i + 1].high &&
            high >= bars[i + 2].high;
        const isPivotLow = low <= bars[i - 1].low &&
            low <= bars[i - 2].low &&
            low <= bars[i + 1].low &&
            low <= bars[i + 2].low;
        if (isPivotHigh)
            highs.push({ index: i, value: high });
        if (isPivotLow)
            lows.push({ index: i, value: low });
    }
...
function resolveHtfTrendState(args) {
    ...
    const longStrong = Number.isFinite(e50) &&
        Number.isFinite(e200) &&
        Number.isFinite(close) &&
        e50 > e200 &&
        close > e50 &&
        swing === "BULL";
    const shortStrong = Number.isFinite(e50) &&
        Number.isFinite(e200) &&
        Number.isFinite(close) &&
        e50 < e200 &&
        close < e50 &&
...
function toSignal(args) {
    ...
    const rawTp = side === "buy" ? entry + risk * OLIKELLA_RRR_TARGET : entry - risk * OLIKELLA_RRR_TARGET;
...
export function evaluateAiMaticOliKellaStrategyForSymbol(symbol, candles, options) {
    const h4 = mergeTimeframeCandles(resampleCandles(candles, H4_MINUTES), options?.h4Candles);
    const h1 = mergeTimeframeCandles(resampleCandles(candles, H1_MINUTES), options?.h1Candles);
    if (h4.length < 40) {
        return {
            state: State.Scan,
            trend: Trend.Range,
    ...
    const h4PriorityLong = [wedgePopLong, baseBreakLong, crossbackLong].filter((item) => Boolean(item?.ok));
    const h4PriorityShort = [wedgePopShort, baseBreakShort, crossbackShort].filter((item) => Boolean(item?.ok));
    ...
    const selectedCross = direction === "BUY"
    ...
    const sweepWickLong = signalBar ? Math.max(0, Math.min(signalBar.open, signalBar.close) - signalBar.low) : Number.NaN;
    const sweepWickShort = signalBar ? Math.max(0, signalBar.high - Math.max(signalBar.open, signalBar.close)) : Number.NaN;
    ...
    const orderbookDataAvailable = Boolean(Number.isFinite(Number(orderflow.bestBid ?? Number.NaN)) &&
        Number.isFinite(Number(orderflow.bestAsk ?? Number.NaN)) &&
        Number.isFinite(Number(orderflow.spreadPct ?? Number.NaN)));
    const microFlowDataAvailable = Boolean(Number(orderflow.buyVolume ?? 0) + Number(orderflow.sellVolume ?? 0) > 0);
    const oiDataAvailable = Number.isFinite(Number(orderflow.openInterestChangePct ?? Number.NaN));
    ...
    const sweepLongOk = Boolean(signalBar &&
        atrReady &&
        ...
        bidDominance >= 0.6);
    const sweepShortOk = Boolean(signalBar &&
        atrReady &&
        ...
        signalBar.volume > signalVolumeSma &&
        askDominance >= 0.6);
    const bodySize = signalBar ? Math.abs(signalBar.close - signalBar.open) : Number.NaN;
    const candleRange = signalBar ? Math.max(signalBar.high - signalBar.low, 1e-12) : Number.NaN;
    const bodyPct = Number.isFinite(bodySize) && Number.isFinite(candleRange)
        ? bodySize / candleRange
        : Number.NaN;
    const displacement = signalBar && prevSignalBar
        ? Math.abs(signalBar.close - prevSignalBar.close)
        : Number.NaN;
    const bosLongOk = Boolean(signalBar &&
        prevSignalBar &&
        atrReady &&
    ...
    const shortFvgSize = signalBar && twoBackSignalBar ? twoBackSignalBar.low - signalBar.high : Number.NaN;
    const longFvgMid = Number.isFinite(longFvgSize) && signalBar && twoBackSignalBar
        ? twoBackSignalBar.high + longFvgSize * 0.5
        : Number.NaN;
    const shortFvgMid = Number.isFinite(shortFvgSize) && signalBar && twoBackSignalBar
        ? signalBar.high + shortFvgSize * 0.5
        : Number.NaN;
    const fvgLongCore = Boolean(atrReady &&
        bosLongOk &&
        Number.isFinite(longFvgSize) &&
        longFvgSize >= signalAtr * 0.2);
    const fvgShortCore = Boolean(atrReady &&
        bosShortOk &&
        Number.isFinite(shortFvgSize) &&
        shortFvgSize >= signalAtr * 0.2);
    const pullbackOverlap = Boolean(signalBar &&
        confirmBar &&
        Math.max(signalBar.low, confirmBar.low) <= Math.min(signalBar.high, confirmBar.high));
    const pullbackVolumeDown = Boolean(signalBar && confirmBar && confirmBar.volume <= signalBar.volume);
    const pullbackImpulse = Boolean(signalBar &&
        confirmBar &&
    ...
    const fvgLongOk = Boolean(fvgLongCore &&
        confirmBar &&
        Number.isFinite(longFvgMid) &&
        confirmBar.low >= longFvgMid);
    const fvgShortOk = Boolean(fvgShortCore &&
        confirmBar &&
    ...
    const orderbookLongOk = Boolean(Number.isFinite(bidAskRatio) &&
        bidAskRatio >= 1.35 &&
            Number(orderflow.topWallRatio ?? 0) >= 2.2 &&
            Number.isFinite(Number(orderflow.spreadPct ?? Number.NaN)) &&
            Number(orderflow.spreadPct) < 0.05);
    const orderbookShortOk = Boolean(Number.isFinite(askBidRatio) &&
        askBidRatio >= 1.35 &&
            Number(orderflow.topWallRatio ?? 0) >= 2.2 &&
            Number.isFinite(Number(orderflow.spreadPct ?? Number.NaN)) &&
            Number(orderflow.spreadPct) < 0.05);
    ...
    const directionPullbackOk = direction === "BUY" ? pullbackLongOk : direction === "SELL" ? pullbackShortOk : false;
    const directionOrderbookOk = direction === "BUY" ? orderbookLongOk : direction === "SELL" ? orderbookShortOk : false;
    ...
    if (microFlowDataAvailable && !directionMicroOk)
        gateFailureReasons.push("MICRO_FLOW_FAILED");
    if (oiDataAvailable && !directionOiOk)
