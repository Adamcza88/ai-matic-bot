...
type SetupFamily = "TREND_PULLBACK" | "NO_TRADE";
...
export type AiMaticBboContext = {
  h1Context: Direction | "RANGE";
  bias4h: Bias4H;
  direction: Direction;
  family: SetupFamily;
  baseScore: number;
  ...
  metrics: {
    ...
    h1Adx: number;
    h4AtrPct: number;
    m5AtrExpansionRatio: number;
    m5VolumeRatio: number;
    ema20: number;
    ema50: number;
...
function overlapRatio(candles: Candle[], lookback = 12) {
  if (candles.length < 3) return Number.NaN;
  const slice = candles.slice(-lookback);
  if (slice.length < 2) return Number.NaN;
  let overlapSum = 0;
  let rangeSum = 0;
  for (let i = 1; i < slice.length; i++) {
    ...
    const overlap =
      Math.max(0, Math.min(curr.high, prev.high) - Math.max(curr.low, prev.low));
...
function resolveStructureDirection(
  ...
  const bear =
    ...
    lastTwoHighs[1].price < lastTwoHighs[0].price &&
    lastTwoLows[1].price < lastTwoLows[0].price;
  if (bull) return "BULL" as const;
  if (bear) return "BEAR" as const;
  return "NONE" as const;
}

function resolveSlopeDirection(candles: Candle[], lookback = 8) {
  if (candles.length < lookback + 1) return "NONE" as const;
  ...
  if (lastClose > first && lastClose >= avg) return "BULL" as const;
  if (lastClose < first && lastClose <= avg) return "BEAR" as const;
  return "NONE" as const;
}

function resolveH1Context(h1: Candle[]) {
  ...
  if (direction === "BULL" && Number.isFinite(overlap) && overlap < H1_OVERLAP_MAX) {
    return { direction: "BULL" as const, overlap };
  }
  if (direction === "BEAR" && Number.isFinite(overlap) && overlap < H1_OVERLAP_MAX) {
    return { direction: "BEAR" as const, overlap };
  }
  return { direction: "RANGE" as const, overlap };
}

function resolve4HBias(h4: Candle[]) {
...
function resolveMtfTrendDirection(candles: Candle[], fastPeriod: number, slowPeriod: number) {
  ...
  if (direction === "BULL" && Number.isFinite(fastNow) && Number.isFinite(slowNow) && fastNow > slowNow) {
    return { direction: "BULL" as const, fastNow, slowNow };
  }
  if (direction === "BEAR" && Number.isFinite(fastNow) && Number.isFinite(slowNow) && fastNow < slowNow) {
    return { direction: "BEAR" as const, fastNow, slowNow };
  }
  return { direction: "NONE" as const, fastNow, slowNow };
}
...
function resolvePullback(
  ...
  for (let i = Math.max(0, m5.length - lookback); i < m5.length; i++) {
    ...
    if (!candle || !Number.isFinite(ema20) || !Number.isFinite(ema50)) continue;
...
function resolveMicroBreak(m5: Candle[], direction: Direction) {
  ...
  const prevHigh =
    lastLow != null ? highs.filter((pivot) => pivot.idx < lastLow.idx).pop() : undefined;
  const prevLow =
    lastHigh != null ? lows.filter((pivot) => pivot.idx < lastHigh.idx).pop() : undefined;
  if (direction === "BULL") {
...
function toTrend(direction: Direction | "RANGE") {
  if (direction === "BULL") return Trend.Bull;
  if (direction === "BEAR") return Trend.Bear;
  return Trend.Range;
}

export function evaluateAiMaticBboStrategyForSymbol(
  symbol: string,
  candles: Candle[],
): EngineDecision {
  if (!candles.length) {
  ...
  const atrExpansionRatio =
    Number.isFinite(atr5) && Number.isFinite(atr5Avg) && atr5Avg > 0
      ? atr5 / atr5Avg
      : Number.NaN;
  ...
  const h4AtrPct =
    Number.isFinite(atr4) && Number.isFinite(h4Close) && h4Close > 0
      ? atr4 / h4Close
      : Number.NaN;
  const hardGatePass =
    Number.isFinite(h4AtrPct) && h4AtrPct >= H4_ATR_HARD_MIN_PCT;
  ...
  const volumeRatio =
    ...
    recentVolumeAvg > 0
      ...
      : Number.NaN;
  const volumeSpikeOk =
    Number.isFinite(volumeRatio) && volumeRatio >= VOLUME_SPIKE_MIN;

  const h1AdxArr = computeADX(
    h1.map((candle) => candle.high),
    h1.map((candle) => candle.low),
    h1.map((candle) => candle.close),
    14,
  );
  const h1Adx = last(h1AdxArr) ?? Number.NaN;

  let baseScore = 0;
  if (direction !== "NONE") baseScore += 25;
  if (pullback.touched) baseScore += 20;
  if (microBreak.breakOk) baseScore += 20;
  if (volumeSpikeOk) baseScore += 10;
  if (atrExpansionOk) baseScore += 10;
  ...
  const stopBuffer =
    Number.isFinite(atr5) && atr5 > 0 ? atr5 * 0.2 : entry * MIN_STOP_PCT;
  let sl = Number.NaN;
  if (direction === "BULL") {
  ...
  const gates: BboGate[] = [
    ...
    },
    {
      ...
      detail:
        Number.isFinite(microBreak.correctionHigh) || Number.isFinite(microBreak.correctionLow)
