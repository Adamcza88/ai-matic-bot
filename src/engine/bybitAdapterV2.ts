...
export interface BybitClient {
  ...
  fetchClosedPnl?: (
    ...
    endTimeMs: number,
    limit?: number
  ) => Promise<{ ok: boolean; list?: { symbol: string; closedPnl: number; execTime: number }[]; error?: string }>;
}
...
export type ClosedPnlFetcherConfig = {
  apiBase: string;
  apiPrefix?: string;
  authToken: string;
  net: "testnet" | "mainnet";
...
export function attachClosedPnlFetcher(client: BybitClient, cfg: ClosedPnlFetcherConfig): BybitClient {
  const apiPrefix = cfg.apiPrefix ?? "/api";
  return {
    ...client,
    fetchClosedPnl: async (startTimeMs: number, endTimeMs: number, limit = 200) => {
      if (!cfg.authToken) return { ok: false, error: "Missing auth token" };
      ...
      try {
        ...
        const mapped = Array.isArray(list)
          ? list.map((r: { symbol?: unknown; closedPnl?: unknown; realisedPnl?: unknown; execTime?: unknown; updatedTime?: unknown; createdTime?: unknown }) => ({
