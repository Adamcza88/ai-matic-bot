 ...
 */
export function computePositionSize(balance, riskPct, entry, sl, feeRate = 0.0006, // 0.06% taker
...
export class TradingBot {
    ...
    computeTrendMetrics(df) {
        ...
        const structureBull = lastHighs.length === 2 &&
            lastLows.length === 2 &&
            lastHighs[1] > lastHighs[0] &&
            lastLows[1] > lastLows[0];
        const structureBear = lastHighs.length === 2 &&
            lastLows.length === 2 &&
            lastHighs[1] < lastHighs[0] &&
            lastLows[1] < lastLows[0];
        let bullScore = 0;
        ...
        if (bullScore >= 3 && bullScore > bearScore)
            ...
            trend = Trend.Bear;
        else if (currentAdx < this.config.adxThreshold)
            trend = Trend.Range;
    ...
    enforceMinimumStop(entry, stop, side, atr) {
    ...
    aggregateOpenRisk() {
        return Object.values(botRegistry).reduce((sum, bot) => {
            ...
            if (!pos)
                return sum;
            const protectiveStop = pos.side === "long"
    ...
    handleManage(ht, lt) {
        ...
        const stopHit = (this.position.side === "long" && (currentPrice <= this.position.stopLoss || currentPrice <= this.position.trailingStop)) ||
     ...
     */
    safeEnterPosition(side, entry, stopLoss, kind = "OTHER") {
        if (!this.canEnter()) {
     ...
     */
    computeSwingStop(df, side, window = 1, backoffAtr = this.config.swingBackoffAtr) {
        const highs = df.map((c) => c.high);
        const lows = df.map((c) => c.low);
        const closes = df.map((c) => c.close);
    ...
    scanForEntryFromFrames(ht, lt) {
        ...
        const emaTrendTouchLookback = Math.max(1, this.config.emaTrendTouchLookback ?? 2);
        const emaTrendConfirmBars = Math.max(1, this.config.emaTrendConfirmBars ?? 2);
        ...
        const emaTrendBias = Number.isFinite(emaTrendNow) && Number.isFinite(htLast?.close)
        ...
        if (emaTouched) {
            ...
            for (let i = confirmStart; i < ht.length; i++) {
                ...
                if (!candle || !Number.isFinite(emaAt))
                    return null;
                if (emaTrendBias === "long" && candle.close <= emaAt)
                    return null;
                if (emaTrendBias === "short" && candle.close >= emaAt)
                    return null;
            }
        }
        const applyEmaTrendGate = (candidate) => {
            if (!candidate)
                return null;
            if (candidate.side !== emaTrendBias)
                return null;
            if (emaTouched && candidate.kind !== "PULLBACK")
                return null;
            return candidate;
        };
        ...
        const price = closes[closes.length - 1];
        const ensureStop = (side, entry, stop) => this.enforceMinimumStop(entry, stop, side, latestATR);
        const entryStopMode = this.config.entryStopMode ?? "atr";
        const entryBackoffAtr = this.config.entrySwingBackoffAtr ?? this.config.swingBackoffAtr;
        ...
        if (trend === Trend.Range && !isTest) {
            ...
            if (latestATR < price * minAtrThreshold)
                return null;
        ...
        if (isTest) {
            ...
            const stop = dir === "long"
                ? resolveEntryStop("long", price, price - this.config.atrEntryMultiplier * latestATR)
                : resolveEntryStop("short", price, price + this.config.atrEntryMultiplier * latestATR);
            return applyEmaTrendGate({ side: dir, entry, stopLoss: stop, kind: "OTHER" });
        }
        return null;
    }
