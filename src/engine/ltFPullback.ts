...
import { Ohlcv, computeEma, computeRsi, findLastPivotHigh, findLastPivotLow } from "./ta";
...
export function detectLtfPullback(
  ...
  lookbackOrOpts: number | PullbackFilters = 2,
  maybeOpts?: PullbackFilters
): PullbackResult {
  const tags: string[] = [];
  ...
  const useRsiFilter =
    opts?.rsiPeriod != null || opts?.rsiMin != null || opts?.rsiMax != null;
  if (useRsiFilter) {
