...
import { computeAtr, findPivotsHigh, findPivotsLow } from "./ta.js";
...
const M15_SWING_NEAR_FIB_TOLERANCE = 5e-3;
const KEY_FIB_PROXIMITY_LIMIT = 0.01;
const RR_MIN = 1.5;
const ATR_BUFFER_MULT = 1.2;
const ATR_FALLBACK_PCT = 3e-3;
...
function findLatestImpulse(trend, swingHighs, swingLows) {
  if (trend === "UP") {
    for (let hi = swingHighs.length - 1; hi >= 0; hi--) {
      const high = swingHighs[hi];
      const low = [...swingLows].reverse().find((candidate) => candidate.idx < high.idx);
  ...
  if (trend === "DOWN") {
    for (let lo = swingLows.length - 1; lo >= 0; lo--) {
      ...
      const high = [...swingHighs].reverse().find((candidate) => candidate.idx < low.idx);
...
function nearestKeyFibDistance(price, fib) {
  ...
  const sorted = key.map((level) => ({ level, distance: pctDistance(price, level) })).sort((a, b) => a.distance - b.distance);
  return {
...
function isBullishPinBar(candle) {
  ...
  const bodySafe = body > 0 ? body : Math.max((candle.high - candle.low) * 0.05, 1e-9);
...
function resolveBreakoutSignals(m15) {
  ...
  const volumeOk = Number.isFinite(volumeAvg) && (curr.volume ?? 0) > volumeAvg * 1;
  return {
...
function evaluateLtfTriggers(m15) {
  if (m15.length < 3) {
    return {
...
function toEngineTrend(trend) {
  if (trend === "UP") return Trend.Bull;
  if (trend === "DOWN") return Trend.Bear;
  return Trend.Range;
}
function evaluateAiMaticProStrategyForSymbol(symbol, candles, _config) {
  if (!candles.length) {
    return {
      state: State.Scan,
      trend: Trend.Range,
  ...
  const fib = impulse ? computeFibLevels({
    trend: trendState,
  ...
  const swingDistanceLong = fib && lastLtfLow ? nearestKeyFibDistance(lastLtfLow.price, fib).distance : Number.POSITIVE_INFINITY;
  const swingDistanceShort = fib && lastLtfHigh ? nearestKeyFibDistance(lastLtfHigh.price, fib).distance : Number.POSITIVE_INFINITY;
  const swingNearFibOk = trendState === "UP" ? swingDistanceLong <= M15_SWING_NEAR_FIB_TOLERANCE : trendState === "DOWN" ? swingDistanceShort <= M15_SWING_NEAR_FIB_TOLERANCE : false;
  ...
  const h4VolatilityOk = Number.isFinite(atr4hCurrent) && Number.isFinite(atr4hAvg20d) && atr4hCurrent >= atr4hAvg20d * 0.8;
  const m15VolatilityOk = !Number.isFinite(atr15Current) || !Number.isFinite(atr15Avg20d) || atr15Current >= atr15Avg20d * 0.5;
  ...
  const triggerValid = trendState === "UP" ? ltfTriggers.longTrigger : trendState === "DOWN" ? ltfTriggers.shortTrigger : false;
  const entry = Number.isFinite(price) ? price : Number.NaN;
  const atrBuffer = Number.isFinite(atr15Current) && atr15Current > 0 ? atr15Current * ATR_BUFFER_MULT : entry * ATR_FALLBACK_PCT;
  let sl = Number.NaN;
  let tp1 = Number.NaN;
  let tp2 = Number.NaN;
  if (trendState === "UP" && fib && Number.isFinite(entry) && entry > 0) {
    const structure = lastLtfLow?.price ?? Math.min(...m15.slice(-10).map((c) => c.low));
    ...
    const structure = lastLtfHigh?.price ?? Math.max(...m15.slice(-10).map((c) => c.high));
  ...
  const rrGateOk = Number.isFinite(rr) && rr >= RR_MIN && Number.isFinite(tp2) && Number.isFinite(sl);
  const gates = [
    {
    ...
    },
    {
      ...
      ok: swingNearFibOk,
      detail: trendState === "UP" ? `dist ${(swingDistanceLong * 100).toFixed(2)}%` : trendState === "DOWN" ? `dist ${(swingDistanceShort * 100).toFixed(2)}%` : "n/a"
    },
    {
    ...
    },
    {
      ...
      ok: volatilityGateOk,
      detail: `H4 ATR ${Number.isFinite(atr4hCurrent) ? atr4hCurrent.toFixed(4) : "NaN"} / avg ${Number.isFinite(atr4hAvg20d) ? atr4hAvg20d.toFixed(4) : "NaN"} | 15m ATR ${Number.isFinite(atr15Current) ? atr15Current.toFixed(4) : "NaN"} / avg ${Number.isFinite(atr15Avg20d) ? atr15Avg20d.toFixed(4) : "NaN"}`
    },
    {
      name: "RR gate >= 1.5",
      ok: rrGateOk,
  ...
  if (!allGatesPass || !side || !Number.isFinite(entry) || !Number.isFinite(sl) || !Number.isFinite(tp2)) {
    return {
      state: State.Scan,
