...
const SettingsPanel = ({ settings, onUpdateSettings, onClose }) => {
    ...
    const coreProfiles = {
        ...
        "ai-matic-x": {
            ...
            description: "HTF EMA cross bias + LTF EMA pullback + micro break close s BBO filtrem a riskem 0.30% equity/trade.",
            notes: [
                ...
                "* Short bias: EMA12(1h) < EMA26(1h)",
                "* No trade: EMA12 ≈ EMA26 (whipsaw zóna) → doporučení: buffer",
                "* prakticky: vynech, když abs(EMA12-EMA26)/price < 0.1% (laditelné)",
                "2) LTF režim (5m) – pullback do trendu",
                "U longu:",
                "* Cena na 5m udělá pullback směrem k “trendové zóně” (konzistentně):",
                "* default jednoduché pravidlo: 5m close se dotkne / zavře pod EMA12(5m) alespoň 1x, ale trend se nezlomí.",
                "* Alternativa (robustnější): pullback do mezery EMA12–EMA26 na 5m (často funguje líp než jedna EMA).",
                "U shortu zrcadlově.",
                "3) Trigger (5m) – micro break (close)",
                "Definice mikro struktury (bez subjektivity):",
                "* Použij pivot: micro swing high/low = fractal s 2 svíčkami vlevo i vpravo (pivot length 2).",
                "Long trigger",
                "1. Po pullbacku vznikne micro swing low (pivot low).",
                "2. Označ poslední micro swing high před tímto low.",
                "3. Vstup, když 5m svíce CLOSE > micro swing high. (to je tvůj “micro break - close”)",
                "Short trigger",
                "* Zrcadlově: po pullbacku pivot high, vstup na CLOSE < micro swing low.",
                "4) Exekuce filtr (BBO)",
                "* Obchod povolen jen když:",
                "* BBO fresh = true",
                "* BBO age < limit (např. BTC/ETH < 800 ms, SOL < 600–800 ms — laditelné)",
                "* volitelně: spread < limit (pokud máš k dispozici)",
                "SL / TP / management (risk-first)",
                "SL (strukturální, nejlogičtější k micro break)",
                ...
                "* TP1 = 1R (zavřít 30–50%)",
                "* TP2 = 2R (zavřít 30–50%)",
                "* Zbytek: trail na 5m pivotech nebo EMA26(5m)",
                "Varianta B (strukturální)",
                "* TP1 = poslední LTF swing level",
                "* TP2 = nejbližší 1h S/R zóna (high/low, supply/demand)",
                "Slabiny / rizika (kde to typicky pálí)",
                "* EMA cross na 1h je náchylný na chop → proto buffer / no-trade zóna.",
                "* Micro break close umí kupovat “poslední push” před otočkou → pomáhá filtr:",
                "* neobchodovat, když je 1h v těsné konsolidaci (ATR komprese)",
                "* nebo když 5m dělá BOS oběma směry (range).",
                "Checklist (rychlá exekuce)",
                "1. 1h: EMA12 > EMA26 (long) / < (short)?",
                "2. 5m: proběhl pullback do EMA12 nebo zóny EMA12–EMA26?",
                ...
                "* např. na 1h: ADX(14) < 18 → no trade",
                "* nebo: ATR(14)/price na 1h pod práh → no trade (komprese)",
                "* Max počet trade/day a max denní loss (např. 3 ztráty = stop).",
            ],
        },
        "ai-matic-scalp": {
