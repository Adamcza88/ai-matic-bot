    ...
    <div className="fixed inset-0 bg-background/80 backdrop-blur-xs flex items-center justify-center z-50">
      <div className="w-full max-w-lg bg-card text-card-foreground rounded-xl border shadow-lg p-6">
        <div className="flex flex-col space-y-1.5 mb-6">
          ...
          <p className="text-sm text-muted-foreground">AI-Matic je uzamčený profil (pouze ke čtení).</p>
        </div>
        ...
        <div className="grid gap-4 py-4">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 rounded-md border border-input px-3 py-2 text-sm ${
                  local.riskMode === "ai-matic" ? "bg-emerald-600 text-white" : "bg-slate-800 text-slate-200"
                }`}
              >
                AI-Matic
              </button>
              <button
                ...
                className={`flex-1 rounded-md border border-input px-3 py-2 text-sm ${
                  local.riskMode === "ai-matic-x" ? "bg-emerald-600 text-white" : "bg-slate-800 text-slate-200"
                }`}
              >
                AI-Matic-X
              </button>
          ...
          <div className="grid gap-2">
            <label className="text-sm font-medium leading-none">Custom Strategy</label>
          ...
          <div className="mt-2 p-3 rounded-lg border border-slate-800 bg-slate-900/40 text-sm space-y-2">
            ...
            <div className="text-xs text-slate-500">
              Parametry: Entry {local.entryStrictness} • Hours {local.enforceSessionHours ? "On" : "Off"} • Base risk{" "}
              {(local.baseRiskPerTrade * 100).toFixed(2)}% • Risk budget {(local.maxPortfolioRiskPercent * 100).toFixed(1)}% /{" "}
              {local.maxOpenPositions} pos • Max alloc {(local.maxAllocatedCapitalPercent * 100).toFixed(1)}% • Max DD{" "}
              {(local.maxDrawdownPercent * 100).toFixed(2)}%
            </div>
          </div>
        </div>
